app.controller('addQuestionsController', [
    ...
    function ($scope, $rootScope, $ajax, $state, $stateParams, $location, $modal, $timeout, Upload) {
        ...
        $scope.data = {
            selectedQuestion: $scope.questions[0]
        };

        if(!$scope.$parent.savedQuestions && $stateParams.id) {
            $ajax.get('/cms/train/selectOneEvaluate', {trainId: $stateParams.id}, function(data) {
                $scope.$parent.test = data.evaluate;
        ...
        $scope.next = function() {
            ...
            $state.go('app.training.test.settings');
        };
