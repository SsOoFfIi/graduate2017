app.controller('marketingToolsDetailController', [
    '$scope',
    '$rootScope',
    '$ajax',
    '$state',
    ...
    function ($scope, $rootScope, $ajax, $state, $stateParams, $location, $modal) {
        ...
        if(!$scope.data[$stateParams.index]) {
            $state.go('app.tools.marketingTools');
            return;
        }
        $scope.currentSelected = $scope.data[$stateParams.index];
    }]);